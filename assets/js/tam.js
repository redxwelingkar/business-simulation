        ...
        for (let i = 0; i < data.length; i++) {
            ...
            for (let key in data[i]) {
                ...
                if (key == 'column1') {
                    ...
                    newCell.innerHTML = '<input class="inputs" type="text" placeholder="Enter Numerical Value" value="' + data[i][key] + '" >';
                    newCell.querySelector("input").addEventListener("change", function () {
...
function populate_total_tam() {
    ...
    for (let i = 1; i < table_TAM.rows.length; i++) {
        const row = table_TAM.rows[i];
        console.log(row);
        console.log(row.cells[1].querySelector("input").value.replace(",", ""));
        total_tam = total_tam + parseInt(row.cells[1].querySelector("input").value.replace(",", ""));
        console.log(total_tam);
